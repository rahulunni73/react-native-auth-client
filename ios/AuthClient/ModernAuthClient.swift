...
extension AuthClient: NativeAuthClientSpec {
  ...
  func getClientInitInfo(requestId: String) -> Promise<String> {
    return Promise { resolve, reject in
      // Implementation for getting client init info
      var result: [String: Any] = [:]
      result["baseUrl"] = Client.getBaseUrl()
      result["clientId"] = Client.getClientId()
      result["encryptionEnabled"] = Client.getIsEncryptionRequired()
  ...
  // MARK: - Additional TurboModule Methods
  
  func cancelRequest(requestId: String) -> Promise<String> {
    return Promise { resolve, reject in
      self.cancelRequest(requestId)
      resolve("{\"message\": \"Request cancelled\", \"requestId\": \"\(requestId)\"}")
    }
  }
  
  func cancelAllRequests() -> Promise<String> {
    return Promise { resolve, reject in
      self.cancelAllRequests()
      resolve("{\"message\": \"All requests cancelled\"}")
    }
  }
}
#endif
