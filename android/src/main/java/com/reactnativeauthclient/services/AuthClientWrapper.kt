...
class AuthClientWrapper(
  ...
  private var sslPinningConfigs: List<SSLPinningConfigHolder.PinningConfig> = emptyList()
  ...
  // Make OkHttpClient mutable so it can be rebuilt with SSL config
  ...
  private fun buildOkHttpClient(): OkHttpClient {
    ...
    // Add SSL pinning if configured
    if (sslPinningConfigs.isNotEmpty()) {
      try {
        builder.certificatePinner(buildCertificatePinner())
        Log.d(TAG, "✅ SSL Pinning enabled for domains: ${sslPinningConfigs.map { it.hostname }}")
        sslPinningConfigs.forEach { config ->
          Log.d(TAG, "   📌 ${config.hostname} (${config.pins.size} pin(s), subdomains: ${config.includeSubdomains})")
        }
      } catch (e: Exception) {
        Log.e(TAG, "❌ Failed to configure SSL pinning", e)
      }
    } else {
      Log.w(TAG, "⚠️ SSL Pinning NOT configured - connections are not pinned")
    }
  ...
  private fun buildCertificatePinner(): CertificatePinner {
    ...
    sslPinningConfigs.forEach { config ->
      config.pins.forEach { pin ->
        // Add pattern for exact hostname
        builder.add(config.hostname, "sha256/$pin")

        // Add pattern for subdomains if enabled
        if (config.includeSubdomains) {
          builder.add("*.${config.hostname}", "sha256/$pin")
        }
      }

      Log.d(TAG, "📌 Certificate pinning configured for ${config.hostname} with ${config.pins.size} pin(s)")
    }

    return builder.build()
  }

  private fun rebuildOkHttpClient() {
    Log.d(TAG, "🔄 Rebuilding OkHttpClient with new SSL configuration")
    _okHttpClient = buildOkHttpClient()
    apiService = null // Force recreation of Retrofit service with new client
  }
  ...
  suspend fun initializeClient(
    ...
    clientId: String,
    passPhrase: String,
    sslConfigs: List<SSLPinningConfigHolder.PinningConfig>?,
    requestId: String
  ): String {
    return withContext(Dispatchers.IO) {
      try {
        ...
        // Store SSL pinning config and rebuild client if provided
        if (!sslConfigs.isNullOrEmpty()) {
          this@AuthClientWrapper.sslPinningConfigs = sslConfigs
          Log.d(TAG, "🔒 SSL Pinning configuration received for ${sslConfigs.size} domain(s)")

          // Rebuild OkHttpClient with new SSL pinning config
          rebuildOkHttpClient()

          result["sslPinningEnabled"] = true
          result["sslPinnedDomains"] = sslConfigs.map { it.hostname }
        } else {
          Log.w(TAG, "⚠️ No SSL pinning configuration provided")
          result["sslPinningEnabled"] = false
        }
  ...
  suspend fun getClientInitInfo(requestId: String): String {
    return withContext(Dispatchers.IO) {
      ...
      result["isConfigured"] = (baseUrl.isNotEmpty())
      result["sslPinningEnabled"] = sslPinningConfigs.isNotEmpty()

      if (sslPinningConfigs.isNotEmpty()) {
        result["sslPinnedDomains"] = sslPinningConfigs.map { it.hostname }
      }
  ...
  fun getConfigurationInfo(): Map<String, Any> {
    return mapOf(
      ...
      "sslPinningEnabled" to sslPinningConfigs.isNotEmpty(),
      "sslPinnedDomains" to sslPinningConfigs.map { it.hostname }
    )
  }
